...
export async function updateWeeklyFacultyStats() {
  try {
    await connectDB();
    console.log("🔹 MongoDB ga ulandi, Weekly Faculty stats cron ishga tushdi");

    const now = new Date();
    const currentWeek = getISOWeek(now); // masalan: "2025-W38"
    ...
    const stats = await SeatBooking.aggregate([
      ...
      },
      {
        $addFields: {
          week: {
            $dateToString: { format: "%G-W%V", date: "$usageStartedAt" }, // ISO haftalar
          },
          usageMinutes: {
            $divide: [
    ...
    await redis.set("facultyWeek", JSON.stringify(facultyData));
    ...
    console.log(
      "✅ Weekly Faculty stats yangilandi va DB+Redisga saqlandi:",
      facultyData
    );
  } catch (error) {
...
export function startWeeklyFacultyStatsCron() {
  // Har haftaning yakshanba kuni 23:59 da ishlaydi
  cron.schedule("10 * * * *", () => {
